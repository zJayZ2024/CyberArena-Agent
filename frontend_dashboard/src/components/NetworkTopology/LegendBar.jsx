...
import { translatePhase } from "../../utils/localization";
      ...
      {phases.map((item, index) => (
        <React.Fragment key={item}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontFamily: T.fontMono, fontSize: 10, color: index < current ? T.red : index === current ? T.amber : T.grayDim, fontWeight: index === current ? 700 : 400, letterSpacing: 0.5 }}>{translatePhase(item)}</span>
          </div>
