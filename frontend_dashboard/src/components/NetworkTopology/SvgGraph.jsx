...
function Tooltip({ cfg, status, atkCnt, defCnt, vulnDetails = {} }) {
  ...
  const lines = [`Zone:     ${cfg.zone.toUpperCase()}`, `Ports:    ${cfg.ports.join(", ") || "none"}`, `Status:   ${st.label}`, `Attacks:  ${atkCnt}   Defenses: ${defCnt}`, ...(vulnEntries.length ? [] : cfg.vulns.map((v) => `Vuln: ${v}`))];
  const w = 220;
  const lh = 14;
  const pad = 10;
  const vulnBlockH = vulnEntries.length * 36 + (vulnEntries.length ? 20 : 0);
  const h = lines.length * lh + pad * 2 + 22 + vulnBlockH;
  const tipX = cfg.x > 600 ? cfg.x - w - 20 : cfg.x + 34;
  const tipY = Math.max(20, Math.min(440 - h - 20, cfg.y - h / 2));
  ...
  return (
    <g transform={`translate(${tipX},${tipY})`} style={{ pointerEvents: "none" }}>
      ...
      {vulnEntries.map(([vid, v], i) => {
        const y = pad + 24 + lines.length * lh + 6 + i * 36;
        return (
          <g key={vid}>
            <rect x={pad} y={y} width={w - pad * 2} height={32} rx={3} fill={T.bg} stroke={T.border} />
            <text x={pad + 6} y={y + 12} fontFamily={T.fontMono} fontSize={8} fill={v.severity === "Critical" ? T.red : v.severity === "High" ? T.amber : T.grayText}>{v.severity}</text>
            <text x={pad + 6} y={y + 24} fontFamily={T.fontMono} fontSize={7} fill={T.grayText}>{vid}</text>
            <text x={w - pad - 6} y={y + 18} textAnchor="end" fontFamily={T.fontMono} fontSize={8} fill={T.grayDim}>exp {(v.exploit_prob * 100).toFixed(0)}%</text>
          </g>
        );
      })}
    </g>
...
function SvgGraph({ round, rounds, idx, hoveredNode, onHoverNode, toast }) {
  ...
  const defEdge = defTarget ? { x1: 830, y1: 220, x2: defTarget.x, y2: defTarget.y } : null;
  ...
  return (
    <div style={{ position: "relative", background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`, overflow: "hidden" }}>
      ...
      <svg viewBox="0 0 880 440" width="100%" height="auto" style={{ display: "block" }} preserveAspectRatio="xMidYMid meet">
        <defs>
        ...
        <rect width="880" height="440" fill="url(#sl)" />
        ...
        <line x1={50} y1={220} x2={86} y2={220} stroke={T.redDim} strokeWidth="1" strokeDasharray="4 3" opacity={0.5} markerEnd="url(#ag)" />
        <line x1={830} y1={220} x2={666} y2={220} stroke={T.blueDim} strokeWidth="1" strokeDasharray="4 3" opacity={0.5} markerEnd="url(#ag)" />
        ...
        {Object.values(NODE_CONFIGS).map((cfg) => {
          ...
          return <NetworkNode key={cfg.id} cfg={cfg} status={nodeStatus(cfg.id)} atkCnt={nd.attack_count ?? 0} defCnt={nd.defense_count ?? 0} isTarget={cfg.id === targetNode} isDefended={cfg.id === defendNode && nodeStatus(cfg.id) === "defended"} hovered={hoveredNode === cfg.id} onHover={onHoverNode} onClick={() => {}} />;
        })}
        {hoveredNode && NODE_CONFIGS[hoveredNode] && (() => {
          const cfg = NODE_CONFIGS[hoveredNode];
          const nd = nodeData(hoveredNode);
          const networkNode = round?.network_nodes?.[hoveredNode] ?? {};
        ...
        <Base x={50} y={220} label="RED BASE" sublabel="ATTACKER" score={score.red} color={T.red} bg={T.redBg} glowColor={T.redGlow} Icon={IconTerminal} />
        <Base x={830} y={220} label="BLUE BASE" sublabel="DEFENDER" score={score.blue} color={T.blue} bg={T.blueBg} glowColor={T.blueGlow} Icon={ICONS.fw} />
